...
static const char rcsid[] =
  "$Id: sc_attach.c,v 1.21 2015/08/30 00:05:21 mjl Exp $";
#endif
...
static void usage(uint32_t opt_mask)
{
  fprintf(stderr,
	  ...
	  "                 [-O options] [-p port] [-P priority] [-R unix]\n"
	  "                 [-U unix]\n");
  ...
  if(opt_mask & OPT_PORT)
    fprintf(stderr, "     -p port to find local scamper on\n");
...
static int check_options(int argc, char *argv[])
{
  ...
  while((ch = getopt(argc, argv, opts)) != -1)
	...
	case 'v':
	  printf("$Id: sc_attach.c,v 1.21 2015/08/30 00:05:21 mjl Exp $\n");
	  return -1;
