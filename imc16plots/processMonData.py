...
'/home/amogh/projects/comcast_ping/code/sync/monitor_list_samples.txt'
	...
	with open(fileName) as monitorList:
		...
		for monitor in bMonitors:
			...
			bordermapDirName = \
'/project/comcast-ping/TSP/adaptive/mon_data/<monitor_name>\
/bdrmap-state/warts/current/'
...
shell=True)).split('\n')
			...
			if len(lsOutput) < 1:
				...
				print monitor + ' contains no .wart data'
				continue
...
folderName + '/*/ 2>/dev/null', shell=True)).split('\n')
				print 'Processing new months for monitor ' + monitor
			...
			for i in dateFilenames:
				temp = i.split('/')
				if len(temp) != 7 or not isInt(temp[-2]):
...
'stat --format="%s" ' + bordermapDirName + wart, shell=True))[:-1])
			...
			for date in uniqueWartFileDates:
				...
				skip = False
				# Skipping completed elements
				for i in dateFilenames:
					if date in i:
						print date + ' already completed'
						skip = True
						break
				if skip == True:
					continue
...
dateFolderLocation + processedWart)
				...
				# sc_bdrMap execution
				print 'Running sc_bdrMap on wart date ' + date
